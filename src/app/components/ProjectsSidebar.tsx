...
// ─── Main component ────────────────────────────────────────────────────────────
export function ProjectsSidebar({ selectedSpace, onSpaceSelect, onClose, activeSection = "project", onSectionChange, onViewChange }: ProjectsSidebarProps) {
    ...
    <div className="w-full md:w-[240px] h-full bg-white border-r border-gray-200 flex flex-col shrink-0">
      ...
      <div className="px-4 pt-4 pb-3 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-1.5 flex-1">
          ...
          <span className="text-[10px] text-gray-400 ml-1">{allProjectSpaces.length}</span>
        </div>
        <button
          onClick={() => {
            setCreateForm({
        ...
        <div className="relative">
          ...
          {showSortMenu && (
            <>
              ...
              <div className="absolute right-0 top-10 w-[180px] bg-white border border-gray-200 rounded-xl shadow-lg shadow-black/8 z-50 py-1 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto py-1.5 px-1.5 pb-16 md:pb-1.5">
        ...
        {onViewChange && (
          <button
            onClick={() => { onViewChange("mywork"); onClose(); }}
            className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl mb-2 bg-cyan-50 hover:bg-cyan-100 active:bg-cyan-200 border border-cyan-100 transition-all group"
          >
            <div className="w-7 h-7 rounded-lg bg-cyan-500 flex items-center justify-center shrink-0">
              <CheckSquare className="w-3.5 h-3.5 text-white" />
            </div>
            <div className="flex-1 text-left min-w-0">
              <p className="text-[12px] font-medium text-cyan-700">Công việc của tôi</p>
